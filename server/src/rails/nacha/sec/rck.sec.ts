/**
 * NACHA Standard Entry Class (SEC): RCK (RePresentedCheckEntry)
 * Description: Re-presentation of NSF returned physical check
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface RePresentedCheckEntryEntry {
  recordTypeCode: '6';
...
export class RePresentedCheckEntryFormatter {
  public static formatEntryDetail(entry: RePresentedCheckEntryEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA RCK record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
