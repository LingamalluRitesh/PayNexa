/**
 * NACHA Standard Entry Class (SEC): CCD (CorporateCreditOrDebit)
 * Description: Commercial vendor payments and intercompany transfers
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface CorporateCreditOrDebitEntry {
  recordTypeCode: '6';
...
export class CorporateCreditOrDebitFormatter {
  public static formatEntryDetail(entry: CorporateCreditOrDebitEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA CCD record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
