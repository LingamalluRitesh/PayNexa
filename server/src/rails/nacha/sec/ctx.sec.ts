/**
 * NACHA Standard Entry Class (SEC): CTX (CorporateTradeExchange)
 * Description: Corporate trading partner payments with ANSI X12 820 addenda
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface CorporateTradeExchangeEntry {
  recordTypeCode: '6';
...
export class CorporateTradeExchangeFormatter {
  public static formatEntryDetail(entry: CorporateTradeExchangeEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA CTX record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
