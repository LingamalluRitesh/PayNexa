/**
 * NACHA Standard Entry Class (SEC): ARC (AccountsReceivableEntry)
 * Description: Consumer paper check converted to ACH at lockbox
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface AccountsReceivableEntryEntry {
  recordTypeCode: '6';
...
export class AccountsReceivableEntryFormatter {
  public static formatEntryDetail(entry: AccountsReceivableEntryEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA ARC record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
