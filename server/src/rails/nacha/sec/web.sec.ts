/**
 * NACHA Standard Entry Class (SEC): WEB (InternetInitiatedEntry)
 * Description: Online consumer debit authorization via web checkout
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface InternetInitiatedEntryEntry {
  recordTypeCode: '6';
...
export class InternetInitiatedEntryFormatter {
  public static formatEntryDetail(entry: InternetInitiatedEntryEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA WEB record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
