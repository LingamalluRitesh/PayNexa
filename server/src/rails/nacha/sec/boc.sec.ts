/**
 * NACHA Standard Entry Class (SEC): BOC (BackOfficeConversionEntry)
 * Description: Consumer paper check converted at POS back office
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface BackOfficeConversionEntryEntry {
  recordTypeCode: '6';
...
export class BackOfficeConversionEntryFormatter {
  public static formatEntryDetail(entry: BackOfficeConversionEntryEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA BOC record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
