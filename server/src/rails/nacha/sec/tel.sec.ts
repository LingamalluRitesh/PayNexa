/**
 * NACHA Standard Entry Class (SEC): TEL (TelephoneInitiatedEntry)
 * Description: Consumer verbal telephone authorization entry
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface TelephoneInitiatedEntryEntry {
  recordTypeCode: '6';
...
export class TelephoneInitiatedEntryFormatter {
  public static formatEntryDetail(entry: TelephoneInitiatedEntryEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA TEL record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
