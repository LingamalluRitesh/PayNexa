/**
 * NACHA Standard Entry Class (SEC): MTE (MachineTransferEntry)
 * Description: Automated teller machine transaction entry
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface MachineTransferEntryEntry {
  recordTypeCode: '6';
...
export class MachineTransferEntryFormatter {
  public static formatEntryDetail(entry: MachineTransferEntryEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA MTE record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
