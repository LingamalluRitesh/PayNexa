/**
 * NACHA Standard Entry Class (SEC): IAT (InternationalAchTransaction)
 * Description: International cross-border transactions with 7 addenda records
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface InternationalAchTransactionEntry {
  recordTypeCode: '6';
...
export class InternationalAchTransactionFormatter {
  public static formatEntryDetail(entry: InternationalAchTransactionEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA IAT record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
