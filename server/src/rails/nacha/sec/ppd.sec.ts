/**
 * NACHA Standard Entry Class (SEC): PPD (PrearrangedPaymentAndDeposit)
 * Description: Direct Deposit of payroll and consumer debits
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface PrearrangedPaymentAndDepositEntry {
  recordTypeCode: '6';
...
export class PrearrangedPaymentAndDepositFormatter {
  public static formatEntryDetail(entry: PrearrangedPaymentAndDepositEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA PPD record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
