/**
 * NACHA Standard Entry Class (SEC): POP (PointOfPurchaseEntry)
 * Description: Consumer paper check converted at retail cash register
 * Compliant with NACHA Operating Rules & Guidelines v2026
 */

export interface PointOfPurchaseEntryEntry {
  recordTypeCode: '6';
...
export class PointOfPurchaseEntryFormatter {
  public static formatEntryDetail(entry: PointOfPurchaseEntryEntry): string {
    const recType = '6';
    ...
    if (line.length !== 94) {
      throw new Error(`Invalid NACHA POP record length: expected 94, got ${line.length}`);
    }
    return line;
  }
}
