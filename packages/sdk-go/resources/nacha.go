...
// NachaService provides access to NACHA ACH batch generation and processing.
...
// NewNachaService initializes the resource service.
func NewNachaService(client HTTPClient, baseURL, apiKey string) *NachaService {
	return &NachaService{
		client: client,
		baseURL: baseURL,
		apiKey: apiKey,
...
type NachaServiceResponse struct {
...
func (s *NachaService) Execute(ctx context.Context, method, endpoint string, payload interface{}) (*NachaServiceResponse, error) {
	...
	var apiResp NachaServiceResponse
